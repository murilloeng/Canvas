...
void examples::objects::spheres(canvas::Scene* scene)
{
	...
	for(uint32_t i = 0; i < n; i++)
	{
		for(uint32_t j = 0; j < n; j++)
		{
			...
			((canvas::objects::Sphere*) scene->object(n * i + j))->color_stroke({1.0f, 1.0f, 1.0f, 0.25f});
			((canvas::objects::Sphere*) scene->object(n * i + j))->color_fill(palette.color(i * n + j, 0, n * n - 1));
		}
	}
}