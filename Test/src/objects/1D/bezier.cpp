...
#include "Canvas/inc/Objects/1D/Bezier.hpp"
...
void examples::objects::bezier(canvas::Scene* scene)
{
	...
	//point 1
	point_1->color_fill("red");
	point_1->position({0, 0, 0});
	//point 2
	point_2->color_fill("red");
	point_2->position({1, 1, 0});
	//point 3
	point_3->color_fill("green");
	point_3->position({1, 0, 0});
	//point 4
	point_4->color_fill("green");
	point_4->position({0, 0.5f, 0});
	//point 5
	point_5->color_fill("green");
