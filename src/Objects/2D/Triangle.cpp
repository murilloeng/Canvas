...
namespace canvas
{
	namespace objects
	{
		...
		//buffers
		void Triangle::vbo_size(uint32_t vbo_counter[]) const
		{
			vbo_counter[0] += 3 * (m_stroke + m_fill);
		}
		void Triangle::ibo_size(uint32_t ibo_counter[]) const
		{
			ibo_counter[2] += 3 * m_fill;
			ibo_counter[1] += 6 * m_stroke;
		}
