...
namespace canvas
{
	namespace objects
	{
		...
		//buffers
		void Quad::vbo_size(uint32_t vbo_counter[]) const
		{
			vbo_counter[0] += 4 * (m_stroke + m_fill);
		}
		void Quad::ibo_size(uint32_t ibo_counter[]) const
		{
		...
		void Quad::ibo_stroke_data(uint32_t** ibo_data) const
		{
