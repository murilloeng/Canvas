...
namespace canvas
{
	namespace objects
	{
		...
		//buffers
		void Circle::vbo_size(uint32_t vbo_counter[]) const
		{
			vbo_counter[0] += m_stroke * m_mesh + m_fill * (m_mesh + 1);
		}
		void Circle::ibo_size(uint32_t ibo_counter[]) const
		{
			ibo_counter[2] += 3 * m_mesh * m_fill;
			ibo_counter[1] += 2 * m_mesh * m_stroke;
		}
