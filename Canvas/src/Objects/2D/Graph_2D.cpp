		...
		void Graph_2D::buffers_data_labels(void) const
		{
			...
			for(uint32_t i = 0; i < n1 + 2; i++)
			{
				...
				for(uint32_t j = 0; j < nc; j++)
				{
					...
					{
						vbo_ptr[4 * counter + k].m_position = {0, 0, 0};
					}
					counter++;
				}
			}
			for(uint32_t i = 0; i < n2 + 2; i++)
			{
				uint32_t nc = sprintf(label, m_tics_format[1], x21 + i * (x22 - x21) / (n2 + 1));
				for(uint32_t j = 0; j < nc; j++)
				{
					for(uint32_t k = 0; k < 4; k++)
					{
			...
			m_scene->font(0)->character('1').coordinates(tc);
			vbo_ptr[0].m_position = {0, 0, 0};
			vbo_ptr[1].m_position = {1, 0, 0};
			vbo_ptr[2].m_position = {1, 1, 0};
			vbo_ptr[3].m_position = {0, 1, 0};
			...
			vbo_ptr[2].m_color = m_frame_color;
			vbo_ptr[3].m_color = m_frame_color;
			vbo_ptr[0].m_texture_coordinates = tc + 2 * 0;
			vbo_ptr[1].m_texture_coordinates = tc + 2 * 1;
			vbo_ptr[2].m_texture_coordinates = tc + 2 * 2;
			vbo_ptr[3].m_texture_coordinates = tc + 2 * 3;
			...
			// 		}
			// 		vbo_ptr += 4;
			// 		position[0] += s1 / font_1->height() * r;
			// 	}
			// }
			// for(uint32_t i = 0; i < n2 + 2; i++)
			// {
			// 	vec3 position = {0, float(i) / (n2 + 1), 0};
			// 	uint32_t nc = sprintf(label, m_tics_format[1], x21 + i * (x22 - x21) / (n2 + 1));
			// 	for(uint32_t j = 0; j < nc; j++)
			// 	{
			// 		//character
			...
			// 		//position
			// 		xc[2 * 0 + 0] = xc[2 * 3 + 0] = position[0] + s2 / font_2->height() * a;
			// 		xc[2 * 2 + 1] = xc[2 * 3 + 1] = position[1] + s2 / font_2->height() * b;
			// 		xc[2 * 1 + 0] = xc[2 * 2 + 0] = position[0] + s2 / font_2->height() * (a + w);
			// 		xc[2 * 0 + 1] = xc[2 * 1 + 1] = position[1] + s2 / font_2->height() * (b - h);
			// 		//vertices
			// 		for(uint32_t k = 0; k < 4; k++)
			// 		{
			// 			vbo_ptr[k].m_color = m_frame_color;
