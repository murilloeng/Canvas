...
namespace canvas
{
	namespace textures
	{
		...
		{
			const uint32_t levels = this->levels();
			glTextureStorage2D(m_id, levels, m_format, m_width, m_height);
		}
		void Texture::generate_mipmaps(void) const
		{
		...
		//levels
		uint32_t Texture::levels(void) const
		{
			const uint32_t m = std::max(m_width, m_height);
			return 1 + (uint32_t) std::floor(std::log2(m));
		}
	}
}